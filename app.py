...
def generate_antenna(times, azimuths, elevations, contact_map, coded_date):
    ...
    azimuths_file = open(f'azimuths_{coded_date}.txt', 'w')
    elevations_file = open(f'elevations_{coded_date}.txt', 'w')
...
def index():
    coded_date = request.args.get('coded_date')

    if not coded_date:
        return "Error: Missing 'coded_date' parameter.", 400 

    start = f'{coded_date[0:4]}-{coded_date[4:6]}-{coded_date[6:8]}T00:00:00Z'
    step_size = 60

    metakernel = 'kernels/mk/cebreros.tm'
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        for mission in missions:
            sc_data = spacecraft(mission.spacecraft, mission.name, mission.acronym, times, ets, step_size)
            filename = f'sc_{mission.spacecraft}_{coded_date}.json'
            zip_file.writestr(filename, json.dumps(sc_data, indent=4))

        # Read and add azimuths.txt
        with open("azimuths.txt", "r") as az_file:
            zip_file.writestr("azimuths.txt", az_file.read())

        # Read and add elevations.txt
        with open("elevations.txt", "r") as el_file:
            zip_file.writestr("elevations.txt", el_file.read())

    antenna_times = list(map(lambda time: time + 'Z', antenna_times))
    doc.append(generate_antenna(antenna_times, azimuths, elevations, contact_list, coded_date))
    ...
    return send_file(zip_buffer, mimetype='application/zip', as_attachment=True, download_name="mission_data.zip")
